...
func (ci *CodeInput) Resolve() (string, error) {
	if ci.Code != "" {
		return ci.Code, nil
	}
	if ci.FilePath != "" {
		return ReadCodeFromFile(ci.FilePath, ci.StartLine, ci.EndLine)
	}
	return "", fmt.Errorf("требуется указать code или file_path")
}
